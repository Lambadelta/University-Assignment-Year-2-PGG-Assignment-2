...
GameModel::GameModel(std::string objFileName)
{
	...
	test.setMeshObject(TEST.packageModelObject(objFileName));
	test.initVAO();
...
void GameModel::Update( float deltaTs )
{
	...
	_modelMatrix = glm::rotate(_modelMatrix, _rotation.y, glm::vec3(1,1,0) );
...
void GameModel::Draw(glm::mat4& viewMatrix, glm::mat4& projMatrix)
{
	...
	glUseProgram( _program );
		...
		glBindVertexArray(test.getVAO());
			...
			glDrawArrays(GL_TRIANGLES, 0,test.getNumVerts());
